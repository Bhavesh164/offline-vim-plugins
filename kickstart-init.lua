...
local function chooseFileFinder()
...
vim.keymap.set('n', '<leader>ff', chooseFileFinder(), { desc = '[S]earch [F]iles' })
...
vim.keymap.set('n', 'X', '<C-v>', { noremap = true }) --for mapping visual block mode
...
-- emmet  (you need to install npm install -g emmet-ls)
...
vim.keymap.set("n", "gV", "`[v`]",{ desc = "select the pasted text"})			    -- select the pasted text again 
...
-- Change directory too (if you want :e somefile to work from that location
vim.api.nvim_create_user_command("EHere", function()
